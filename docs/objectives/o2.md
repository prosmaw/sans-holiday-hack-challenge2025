...
!!! tip "Prevention"
    - Use absolute paths (e.g. `/usr/bin/free`) for every command invoked inside a privileged script, or explicitly set a locked-down `PATH` (e.g. `PATH=/usr/bin:/bin`) at the top of the script so it can't pick up a binary from a user-writable directory.
    - Avoid `NOPASSWD` sudo entries for scripts that shell out to other binaries; if elevated automation is required, keep the script minimal, restrict who can run it, and review `sudo -l` output regularly for overly permissive entries.
