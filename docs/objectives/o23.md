...
1-Wire uses a single data line (DQ) with no separate clock, so bits are encoded by the width of low pulses. I noticed that short low pulse are `6 µs` and represent a bit 1 while long low pulses are `60 µs` and represent a bit 0.

A voltage `v` reading `0` corresponds to "High" idle state, and `1` means the line is low.

Under the message part of the devtools network section for the qd request, the data format is as follows:
...
- Two Websockets streams are collected: SDA (`"data-bit"` or `"address-bit"`) and SCL (`"data-sample"` or `"address-sample`)
- Streams close when the `"bus-idle"` marler is seen twice with `t=0`, indicating a full broadcast.
