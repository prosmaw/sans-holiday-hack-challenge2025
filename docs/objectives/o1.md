...
```
After running the regex, these IP addresses have been collected and non-suspicious IPs like the neighborhood mail server IP have been excluded from the IOC list.
Note: this screenshot is from the initial regex return. The IP 10.0.0.5 was excluded as an IOC.
