...
For every actions that called the `\gnomeU` endpoint, I got sent to the proxy part on Burp Suite and had to forward or drop the requests.
...
I followed the gnome hint about dev notes and got access to the `dev/dev_notes` endpoint that initialy led to a 403 page.
