---
icon: material/text-box-outline
---

# Owner

**Difficulty**: :fontawesome-solid-star::fontawesome-regular-star::fontawesome-regular-star::fontawesome-regular-star::fontawesome-regular-star:<br/>
**Direct link**: [Owner](https://2025.holidayhackchallenge.com/badge?section=objective&id=objTokenExposure)

## Objective

!!! question "Request"
    Help Goose James near the park discover the accidentally leaked SAS token in a public JavaScript file and determine what Azure Storage resource it exposes and what permissions it grants.

??? quote "James"
    ![Screenshot 2026-04-21 132948](../img/objectives/o10/Screenshot%202026-04-21%20132948.png)


## Solution
James asked me to audit the HOA Azure's RBAC config for permanently assigned Owner roles.

The terminal session introduced the task and the Azure CLI environment.

![Screenshot 2026-04-21 144456](../img/objectives/o10/Screenshot%202026-04-21%20144456.png)

The first task was to list all subscriptions names with the command:

~~~bash linenums="1" title="List subscription names"
az account list --query "[].name"
~~~

The output showed the subscription names available in the tenant.

![Screenshot 2026-04-21 144534](../img/objectives/o10/Screenshot%202026-04-21%20144534.png)

The next task was to list enabled subscriptions by ID and check Owner role associated with each subscription.

I first used the following command to list enabled subscriptions with IDs:


~~~bash linenums="1" title="List enabled subscriptions"
az account list --query "[?state=='Enabled'].{Name:name, ID:id}"
~~~

I later used this command to list owner for each subscription replacing the appropriate subscription ID:

~~~bash linenums="1" title="List Owner role assignments"
az role assignment list --scope "/subscriptions/2b0942f3-9bca-484b-a508-abdae2db5e64" --query "[?roleDefinition=='Owner']"
~~~

The results showed the Owner assignments for the first subscription (ID not shown on screenshot).

There is effectively a group present for the Owners permission.

![Screenshot 2026-04-21 144702](../img/objectives/o10/Screenshot%202026-04-21%20144702.png)

The next task was to check Owner permission on the second subscription:

~~~bash linenums="1" title="Owners for subscription 4d9d..."
az role assignment list --scope "/subscriptions/4d9dbf2a-90b4-4d40-a97f-dc51f3c3d46e" --query "[?roleDefinition=='Owner']"
~~~

The output listed Owner permission for the subscription as well.

![Screenshot 2026-04-21 144733](../img/objectives/o10/Screenshot%202026-04-21%20144733.png)

The next task was to check Owner assignments on the third subscription:

~~~bash linenums="1" title="Owners for subscription 065c..."
az role assignment list --scope "/subscriptions/065cc24a-077e-40b9-b666-2f4dd9f3a617" --query "[?roleDefinition=='Owner']"
~~~

The output identified a group named "IT Admins".

![Screenshot 2026-04-21 152942](../img/objectives/o10/Screenshot%202026-04-21%20152942.png)

The next task was to list members of "IT Admins" group using the command:

~~~bash linenums="1" title="List group members (6b98...)"
az ad member list --group 6b982f2f-78a0-44a8-b915-79240b2b4796
~~~

The results showed the users and service principals in that group and another group ID.

![Screenshot 2026-04-21 153116](../img/objectives/o10/Screenshot%202026-04-21%20153116.png)

The next task was to list members of the nested group ID using the command:

~~~bash linenums="1" title="List group members (631e...)"
az ad member list --group 631ebd3f-39f9-4492-a780-aef2aec8c94e | less
~~~

The output revealed the identity of the IT admin user.

![Screenshot 2026-04-21 153401](../img/objectives/o10/Screenshot%202026-04-21%20153401.png)

The challenge completed after identifying the permanently assigned Owner role.

![Screenshot 2026-04-21 153422](../img/objectives/o10/Screenshot%202026-04-21%20153422.png)




## Response

!!! quote "James"
    You found the permanent assignments! CLUCK!
    See, I'm not crazy - the security really WAS misconfigured. Now maybe I can finally get some peace and quiet...
